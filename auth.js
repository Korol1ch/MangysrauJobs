...
router.delete('/me', authRequired, (req, res) => {
  ...
  // Если работодатель — удаляем его вакансии и отклики на них
  const jobs = db.prepare('SELECT id FROM jobs WHERE user_id = ?').all(userId);
  jobs.forEach(j => {
    db.prepare('DELETE FROM applications WHERE job_id = ?').run(j.id);
  });
